...
function params(where, sort) {
  ...
  } else { // where.like will generate only params.key entry
  ...
  if (sort) { // add sort options
    ...
    console.log("params() ", where, " -- ", sort, " --> ", params);
  }

  return params;

  function hasLike(where) {
    return where.like && 'object' == typeof(where.like)
  }
}

function likeValue(like) {
  var startKey = [];
  var endKey = [];
  Object.keys(like).sort().forEach(function(key) {
    var value = like[key];
